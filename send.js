...
async function buildOfferPdf(data) {
  ...
  (function () {
    var priceStr = formatNOK(data.model.price);
    page.drawText(data.model.name, { x: margin, y: y, size: 15, font: bold, color: ink });
    var priceWidth = bold.widthOfTextAtSize(priceStr, 15);
    page.drawText(priceStr, { x: pageWidth - margin - priceWidth, y: y, size: 15, font: bold, color: accent });
    y -= 26;
  })();
  ...
  function lineItem(label, item) {
    ...
    page.drawText(label + ": " + item.name, { x: margin, y: y, size: 12, font: font, color: ink });
