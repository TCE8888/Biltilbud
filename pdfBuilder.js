// Shared PDF-building and offer-validation logic used by BOTH api/send.js
// (emails the offer) and api/preview.js (renders the same PDF for local
// printing, no email involved). Keeping this in one place is deliberate:
// two independent copies of this logic have drifted out of sync before
// (the financing-fee bug), so from here on there is exactly one place that
// knows how an offer turns into a PDF.
const fs = require("fs");
...
async function buildOfferPdf(data) {
  ...
  text((data.customer && data.customer.name) || "–", { size: 13, gap: 16 });
  if (data.customer && data.customer.phone) text("Tlf: " + data.customer.phone, { size: 11, color: muted, gap: 14 });
  if (data.customer && data.customer.email) text("E-post: " + data.customer.email, { size: 11, color: muted, gap: 14 });
  y -= 6;
  hr();
...
// Validates and normalizes a raw request body into everything buildOfferPdf
// (and, for api/send.js, the email-sending code) needs. Shared by
// api/send.js and api/preview.js so the two can never compute a different
// total or a different financing example from the same input again.
// Returns { error: "missing_model" | "missing_customer_email" } on bad
// input, or the full prepared offer otherwise.
function prepareOffer(body, opts) {
  opts = opts || {};
  ...
  if (opts.requireCustomerEmail && (!customer || typeof customer.email !== "string" || !customer.email.trim())) {
    return { error: "missing_customer_email" };
  }
  ...
  return {
    ...
    dateStr, validUntilStr, cleanSellerName, cleanSellerPhone, replyToEmail, sellerLine,
    customer: customer || {}, note: note || "", companyName: companyName || "Autosalg",
...
// Loads and, if necessary, converts (webp → png) the car photo for the
// given clean paint entry. Best-effort: any problem (missing file, bad
// convert) just means no photo, never a failed request.
async function loadCarImage(cleanPaint) {
  if (!cleanPaint || !cleanPaint.image) return { carImageBytes: null, carImageExt: null };
  var imgInfo = safeCarImagePath(cleanPaint.image);
  if (!imgInfo) return { carImageBytes: null, carImageExt: null };
  try {
    var rawImageBytes = fs.readFileSync(imgInfo.full);
    if (imgInfo.ext === "webp") {
      // pdf-lib can only embed JPEG/PNG, so webp is converted to PNG first.
      // Requiring sharp here (rather than at the top of the file) means a
      // problem with that optional dependency only costs the photo, not
      // the whole request — everything else still works.
      var sharp = require("sharp");
      var pngBytes = await sharp(rawImageBytes).png().toBuffer();
      return { carImageBytes: pngBytes, carImageExt: "png" };
    }
    return { carImageBytes: rawImageBytes, carImageExt: imgInfo.ext };
  } catch (e) {
    return { carImageBytes: null, carImageExt: null }; // file not uploaded yet, or couldn't convert
  }
}

module.exports = { LOGO_BYTES, formatNOK, monthlyPayment, buildOfferPdf, prepareOffer, loadCarImage };
